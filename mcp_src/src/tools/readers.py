...
async def handle_get_campaign(arguments: dict) -> list[TextContent]:
    ...
    if not campaign_id:
        return [TextContent(text=err_required("campaign_id"))]
...
async def handle_list_npcs(arguments: dict) -> list[TextContent]:
    ...
    if not npcs_data:
        return [TextContent(text="No NPCs found in this campaign.")]
...
async def handle_get_npc(arguments: dict) -> list[TextContent]:
    ...
    if not npc_name:
        return [TextContent(text=err_required("npc_name"))]
...
async def handle_get_combat_status(arguments: dict) -> list[TextContent]:
    ...
    if not combat_data:
        return [TextContent(text="No active combat.")]
...
async def handle_get_bestiary(arguments: dict) -> list[TextContent]:
    ...
    if not bestiary_data:
        return [TextContent(text="No bestiary found. Create enemy templates with create_bestiary_entry.")]
